...
def compute_hoppe(points, normals, volume, number_cells, min_grid, length_cell):
    ...
    for (i,m) in enumerate(volume):
        #print(p)
        print(list(m))
        print(list(m)*length_cell)
        p = min_grid+list(m)*length_cell*m
        n_neighbor = tree.query(p,1)
        neighbor = points[n_neighbor]
        normal = normals[n_neighbor]
        result[i] = np.dot(normal,(p-neighbor).T)
    return result

				
# EIMLS surface reconstruction
def compute_eimls(points,volume,number_cells,min_grid,length_cell):
    k = 10
    result = np.zeros(volume.shape)
    tree = KDTree(points)
    for (i,m) in enumerate(volume):
        ...
        print(list(m))
        print(list(m)*length_cell)
        #p = min_grid+list(m)*length_cell
        n_neighbors = tree.query(p,k)
        neighbors = points[n_neighbors]
        normals = normals[n_neighbors]
        result[i] = np.sum(np.dot(normals,(neighbors+p*(-1)).T)*np.exp(-np.dot(neighbors+p*(-1),((neighbors+p*(-1)).T))/h^2))/np.sum(np.exp(-np.dot(neighbors+p*(-1),((neighbors+p*(-1)).T))/h^2))    	
    return result
...
if __name__ == '__main__':
    ...
    verts, faces, normals, values = measure.marching_cubes_lewiner(volume, level=0.0, spacing=(length_cell[0],length_cell[1],length_cell[2]))
