...
def compute_hoppe(points,normals,volume,number_cells,min_grid,length_cell):
    ...
    for i in range(0,number_cells+1):
        for j in range(0,number_cells+1):
            for k in range(0,number_cells+1):
                x = np.copy(min_grid) + length_cell*np.array([i,j,k])
                ind_nearests = tree.query([x],k=1,return_distance=False)[0]
                volume[i,j,k] = np.dot(normals[ind_nearests[0],:],x-points[ind_nearests[0],:])
...
def compute_eimls(points,normals,volume,number_cells,min_grid,length_cell):
    ...
    for i in range(0,number_cells+1):
        for j in range(0,number_cells+1):
            for k in range(0,number_cells+1):
                x = np.copy(min_grid) + length_cell*np.array([i,j,k])
                ind_nearests = tree.query([x],k=kmax,return_distance=False)[0]
                h = max(0.003, np.linalg.norm(x - points[ind_nearests[0], :]) / 4)
                sum_weight = 0.0
                imls_x = 0.0
                for t in range(0,ind_nearests.size):
                    weight = np.exp(-1.0*np.linalg.norm(x - points[ind_nearests[t], :])**2/h**2)
                    sum_weight = sum_weight + weight
                    imls_x = imls_x + np.dot(normals[ind_nearests[t],:],x-points[ind_nearests[t],:])*weight
                imls_x = imls_x / sum_weight
                volume[i,j,k] = imls_x
